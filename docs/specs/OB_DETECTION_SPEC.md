...
> Note : confrontation au canon **BOUCLÉE** (2026-06-18/19, cf. mémoire `ob-spec-vs-vault-confrontation`). **D1·D2·D3 et R1–R17 tous tranchés/résolus côté doctrine** — chaque décision sourcée + vérifiée à la main au transcript. **R16 (polarité) et R17 (cycle de vie) sont désormais CÂBLÉS et validés en double** dans `OB_Detector_v2.11.txt`. Ne restent, tous **NON-doctrinaux** : **R6** (géométrie fine du gap → validation visuelle TV) ; **inférences à valider** (formule morphologie base §T4 `body<mèche`) ; **câblage** résiduel du modèle de force (dégradations liquidité/contre-swing, cascade §2.6 — engineering). Tout 🟡/⚠️ résiduel relève de l'un de ces trois.
