...
- **TROUS / à décider (non tranchés par le vault)** : (1) plancher de dégradation (pas de clamp canon) ; (2) **force de BASE de l'OB intérieur** — le détecteur utilise le `tf0` de la leg ; le multi-TF complet (base propre à l'intérieur) est **différé** ; (3) **mitigation / cycle de vie** des OB intérieurs (tient/casse, §T1-bis) — différé ; (4) dégradations **liquidité / contre-swing / jeu interne** (§3.2 table, lignes `❌ manquant`) pas encore câblées ; (5) cascade §2.6 (Zone@−½, ABA@−1) côté détecteur ; (6) exception **ordre des cassages** (`mpkBAJCdkgs @ 01:21:58` : hauts cassés avant bas → le mini-swing perd sa polarité). **Ne pas inventer.**
...
| R16 | Mécanisation de la polarité + câblage §4.4 (§3.2) | ✅ **résolu + CÂBLÉ 2026-06-19 (PAYOFF)** : polarité = **localisation** (§2.5), pas un proxy — **extrême = NÉGATIF**, **intérieur = POSITIF**. Proxy « visibilité TF−0,5 » (touches ≥3 / pivot TF−0,5) **codé puis ABANDONNÉ** (floode). Intérieur localisé par **micro-pivots du TF chart** (barres exactes, pas `request.security`). Table §4.4 **câblée déterministe** dans `OB_Detector_v2.10.txt` (extrême −4/−2/−4 · intérieur −2/0/−4). **Validé en double** : arithmétique demi-paliers + agent `quant-viz-verifier` (24/24, §4.4 dans la grille, monotonie extrême≥intérieur, placement vérifié). Restent différés (engineering, non-doctrine) : force de base multi-TF de l'intérieur, mitigation des OB intérieurs, dégradations liquidité/contre-swing, cascade §2.6, exception ordre des cassages |

> Note : confrontation au canon **BOUCLÉE** (2026-06-18/19, cf. mémoire `ob-spec-vs-vault-confrontation`). **D1·D2·D3 et R1–R16 tous tranchés/résolus côté doctrine** — chaque décision sourcée + vérifiée à la main au transcript. **R16 (polarité) est désormais CÂBLÉ et validé en double** dans `OB_Detector_v2.10.txt`. Ne restent, tous **NON-doctrinaux** : **R6** (géométrie fine du gap → validation visuelle TV) ; **inférences à valider** (formule morphologie base §T4 `body<mèche`) ; **câblage** résiduel du modèle de force (dégradations liquidité/contre-swing, cascade §2.6, mitigation des OB intérieurs — engineering). Tout 🟡/⚠️ résiduel relève de l'un de ces trois.
