...
- **Config 4** (énergie inversée → OB d'origine seul) et **Config 5** (contre-structure → « OB du milieu ») : **= la séquence CHoCH-INVERSÉE** (« casse-bas PUIS recasse-haut »), **confirmées verbatim 2026-06-19** `NOAGAfn5VhQ 05:21:16–05:22:55` : *« tous les swings intermédiaires seront de la liquidité… le seul endroit où rentrer = l'order bloc d'origine… le jeu inversé = l'order block du milieu qui fait office d'order block principal »*. **RÉSOLUTION (corrigée 2026-06-19 après re-vérif stricte — j'avais d'abord sur-affirmé « pas de force »)** : la CHoCH-inversée a **DEUX dimensions** : **(a) FORCE** — `choch_energie_inversee` formal `force_initiale(CHoCH) < force_initiale(BoS-BoS)`, réduction RÉELLE réconciliée au **`cassage_multiplicateur`** (+2 demi-paliers/cassage ; `@ 05:20:48` « TF+1 pour casser TF0, TF+2 pour casser ça ») ; **(b) LOCALISATION** — `@ 05:21:16` « les intermédiaires = liquidité → OB re-localisé : config 4 = OB d'origine seul, config 5 = OB du milieu ». ⚠️ Le **sous-modèle de force par cassage est ABSENT du détecteur** (qui utilise `leg-size→tf0`, sans comptage de cassages) → la dim. (a) n'a **pas de hook** sans bâtir d'abord le `cassage_multiplicateur` (autre modèle de force). Le **÷2 fausse-cassure** (`fausse_cassure_division`, `FFxKFvuYxrM 00:55:54` : « un 1-2 redevient un 1 ») relève du **même** sous-modèle cassage ; la porte cassure-corps **A2 empêche** l'OB-mèche (REJET, plus strict que le ÷2 qui garde à moitié) — **liée mais ≠** le ÷2. → À câbler côté force : le `cassage_multiplicateur` (gros, autre modèle). À câbler côté localisation : **configs 4/5** (contre-structure → OB du milieu), structurel.
...
> **⚠️ COUVERTURE DU DÉTECTEUR `OB_Detector_v2.13` vs la « Méthode complète » canon (force-energie.md §3, 6 ÉTAPES) — constat vérifié 2026-06-19 (re-lecture stricte, corrige une sur-affirmation antérieure de l'audit R19)** :
> | Étape canon | Détecteur |
> |---|---|
> | **1.** TF visible (retracement ×ATR → TF) | ✅ `tf0 = leg-size→cascade` (⚠️ mesure la LEG/impulsion, le canon le **RETRACEMENT** — cf. SWING_SEQUENCE_SPEC §1.1) |
> | **2.** Chaîne demi-paliers (vraie force SSI chaîne ininterrompue, sinon **FORCE FANTÔME** — `-ePJRgURtOA 02:04:06` « faux H4 ») | ❌ **NON vérifié** → peut afficher une force fantôme |
> | **3.** Cassages **+2 demi-paliers** (1 cassage +2, 2 = +4, plafond 2) — `cassage_multiplicateur` sur **force_courante** | ❌ **NON implémenté** (pas de comptage de cassages) |
> | **4.** Fausse cassure **÷2** (mèche → force ÷2) — `fausse_cassure_division` | ⚠️ la porte cassure-corps A2 **REJETTE** l'OB-mèche (plus strict, **≠** le ÷2 qui garde à moitié) |
> | **5.** Dégradations §4 (épaisseur/polarité/liquidité/contre-swing) | ✅ **partiel** (les câblées : §4.4, #1c, #4) |
> | **6.** Comparer `force_courante` vs `force_zone` (tient/casse) | ❌ cartographie seule (intentionnel) |
> **Conclusion** : le détecteur ne calcule que **l'étape 1 + une partie de l'étape 5**. Les **étapes 2 (chaîne/force-fantôme), 3 (multiplicateur de cassages), 4 (÷2)** sont **manquantes** — elles relèvent du modèle de force **multi-TF/structurel** (cohérent avec §2.6 « single-TF ne couvre que l'étape 1 »). La **CHoCH-inversée**, le **÷2 jeu-interne** et la **force-fantôme** vivent TOUS dans ces étapes différées. → R19 « cœur de force fidèle » = fidèle sur les *pièces* implémentées, mais la **méthode complète est incomplète** (3 étapes). Registre §4 : **R20**.
...
| R14 | Localisation de l'OB d'origine dans la séquence (§2.5) | ✅ résolu 2026-06-19 : grille « **intérieur → 1ᵉʳ micro-swing / extérieur → 1ᵉʳ gros swing / même TF imbriqué → liquidité** » (`NOAGAfn5VhQ@02:56–03:02`, transcript + visuel « prenez la photo »). **Corrige le timestamp vault erroné `grille_ob_origine`@05:15.** Configs 1/2/3/6 déterministes ; **4/5 = la séquence CHoCH-inversée, CONFIRMÉES 2026-06-19** (`NOAGAfn5VhQ 05:21–05:22`). **Corrigé après re-vérif** : la CHoCH-inversée a **2 dimensions** — (a) FORCE (réduction `force(CHoCH)<force(BoS-BoS)` via `cassage_multiplicateur` +2/cassage, **absent du détecteur** leg-size→tf0 → pas de hook) ; (b) LOCALISATION (config 4 OB d'origine seul / config 5 OB du milieu, codable structurellement). Le ÷2 « jeu interne » (fausse cassure-mèche, `fausse_cassure_division`) relève du **même sous-modèle cassage** ; A2 (cassure-corps) **empêche** l'OB-mèche (rejet, plus strict, **≠** le ÷2). À câbler : `cassage_multiplicateur` (force, gros) + configs 4/5 (localisation) |
...
| R19 | **Audit de conformité doctrinale v2.12** (revue complète CODE↔CANON, 2026-06-19) | ✅ **réalisé** : 3 confrontations indépendantes `doctrine-vault-operator` (≥7 preuves verbatim chacune) **+ re-vérification main aux transcripts bruts**. **Cœur du modèle = FIDÈLE** (cascade demi-paliers + √2, table §4.4, seuils épaisseur/corps, dégradations #1c/#4, cycle de vie conjonctif, polarité=localisation, ATR233). **3 « déviations » force/mesure DISSOUTES à la vérif transcript** : (C) §4.4 — code FIDÈLE (positive −1/négative −2 multi-sourcé `24tpbSEAXeA 01:14:25`+`01:43:04`, `v0uou9miWnQ 00:27:40`/`00:34:33`) ; **BUG dans le VAULT — CORRIGÉ 2026-06-19 (feu vert humain)** : l'axiome YAML `degradation_zone_par_polarite` avait un `formal` erroné (`positive_visible→−2`) qui contredisait ses propres citations ; **réaligné sur les transcripts** (`positive→−1` normale / `0` épaisse, `negative→−2`/`−1`, très-ép −2 ; citations `24tpbSEAXeA 01:14:25`+`01:43:04`, `v0uou9miWnQ 00:34:33`, `6AyQ-UFocFg 00:06:32-45`) — le code n'a PAS bougé (il était juste). (B2) mesure d'épaisseur — **SOURCÉE** : `FQO8F7WmlGY 00:05:19` « énergie du plus bas du corps jusqu'au plus haut » = exactement `hi−min(o,c)`. (B1) mort par traversée — **PLEINEMENT FIDÈLE** : `Kp68Daasc6I 01:09:40` « si la bougie ferme au-dessus de la mèche, ça c'est plus rien » = exactement le `close > top` du code (la « mèche » d'un OB d'offre = le haut = `top`). **VÉRIFICATION 100 % (2026-06-19, re-lecture main de CHAQUE citation par l'opérateur, pas seulement les agents)** : tous les points FIDÈLES reconfirmés verbatim — cascade `6eL9OUdSc94 00:43:18` (« saute de demi-temps → H4,H1,M15 ») + √2 `00:37:32` (« divise la volatilité par 2 ») ; seuils épaisseur `24tpbSEAXeA 01:13:16` (0,75–1,5) + très-ép `WO1gp3sk5U0 00:22:45` (≥2,7) ; corps `FQO8F7WmlGY 00:03:08` (0,3–0,63 ; >0,65 rejeté) ; mort 3-fois conjonctive `FQO8F7WmlGY 00:11:01` + 1er test ; affiné=1-touche `xFnFjopAzz8 01:55:43` ; mèche comprise `rwRNzxzzr10 00:15:05` ; contre-swing `rwRNzxzzr10 00:16:56` (1–1,5 temps) ; **localisation=polarité = mot de Garry** `NOAGAfn5VhQ 03:02:00` (« affaire de polarité : TF−1 dans TF0 ») + 1er micro-swing `02:59:39`. **Déviations RÉELLES restantes = le DÉCLENCHEUR (cluster A)** : (A2) ~~pas de vérif `close>P_high`~~ **CORRIGÉ v2.13 (feu vert humain, transcript)** : PORTE cassure-corps `reqCorps` — l'OB n'est créé que si une CLÔTURE a franchi le swing cassé (`close<prevLow` bear / `close>prevHigh` bull), sinon = fausse cassure/mèche → rejet (`cassage-de-structure.md` « vraie cassure = corps qui clôture au-delà » ; `0OeOoBj-Hb0 00:20:25`, `jevVat55Svg 00:28:23`). Vérifié H1 : une zone wick (~78,3k) filtrée, cœur validé préservé (porte additionnelle). ; (A3) ~~pas de compteur « 1-2 »~~ **DISSOUS 2026-06-19 (re-lecture code, agents avaient sur-flaggé)** : le détecteur **implémente DÉJÀ le « 1-2 »** via la porte `pBrkDir` — `f_OB(break-1)` n'est appelé que `if pBrkDir == ±1` (le break précédent était DE MÊME SENS), donc l'OB n'est créé que sur la **2ᵉ cassure consécutive même sens** (`jevVat55Svg 00:28:23` « une fois, deux fois → identification de l'OB » ; `c_m3ZfUKZdE 03:14:08` 1=choche/2=externe). CHoCH isolée (sans 2ᵉ cassure) → **aucun OB** = canon `choch_definition` « doit être confirmé par un BoS externe ». OB ancré sur l'origine du break-1 = `swing_avant_choch`. Les agents avaient lu `isHH/isLL` (test par swing) et manqué `pBrkDir` (compteur 1-2). ; (A1) filtre `leg ≥ 3 ATR` = proxy de taille (décision de planif humaine ; canon = structure « swing majeur »). **Déclencheur : A2 corrigé (cassure-corps v2.13) ; A3 déjà fidèle (pBrkDir) ; reste A1 (filtre de bruit, choix humain) + l'énergie CHoCH-inversée `energie_choch_inversee` = dégradation de FORCE distincte, non câblée (différée).** Non vu verbatim (à sourcer) : cellule §4.4 « épaisse=0/−1 » (dérivée du « jeu d'un temps au-dessus » `24tpbSEAXeA 01:13:24`) ; seuil de code `ratio>4` (calibration leg→TF, non sourcée). Mineurs assumés : `reste<0,45` (assemblage de 2 notions), coût « fine », pas de cas « ambiguë », `degSeq` partagé, épaisseur intérieur en high−low (à harmoniser). |

| R20 | **Complétude du modèle de FORCE** (détecteur vs « Méthode complète » force-energie.md §3, 6 étapes) | ⚠️ **constat vérifié 2026-06-19 (re-lecture stricte, corrige une sur-affirmation de R19)** : le détecteur ne calcule que **l'étape 1 (TF visible = `leg-size→cascade`) + une partie de l'étape 5 (dégradations §4.4/§3.2 câblées)**. **MANQUENT** : **étape 2** (chaîne demi-paliers → sinon **FORCE FANTÔME**, `-ePJRgURtOA 02:04:06` « faux H4 ») — **NON vérifiée** ; **étape 3** (`cassage_multiplicateur` +2 demi-paliers/cassage, `force_courante += 2×min(cassages,2)`) — **NON implémentée** ; **étape 4** (fausse cassure **÷2**, `fausse_cassure_division`) — la porte A2 **rejette** l'OB-mèche (≠ le ÷2 qui garde à moitié) ; **étape 6** (tient/casse) — cartographie seule (intentionnel). ⚠️ Étape 1 mesure la **LEG**, le canon le **RETRACEMENT** (SWING_SEQUENCE_SPEC §1.1). La **CHoCH-inversée**, le **÷2** et la **force-fantôme** vivent dans les étapes 2/3/4 manquantes (modèle de force **multi-TF/structurel**, différé — cf. §2.6 « single-TF ne couvre que l'étape 1 »). → **R19 « cœur de force fidèle » à nuancer** : fidèle sur les pièces implémentées (mapping cascade, table §4.4, dégradations câblées), mais la méthode complète est **incomplète** (3 étapes). LEÇON (l'humain doutait, à raison) : ne pas affirmer « le modèle de force est fidèle » sans confronter la **méthode complète à 6 étapes**. |

> Note : confrontation au canon **BOUCLÉE** (2026-06-18/19, cf. mémoire `ob-spec-vs-vault-confrontation`). **D1·D2·D3 et R1–R18 tous tranchés/résolus côté doctrine** — chaque décision sourcée + vérifiée à la main au transcript. **R16 (polarité), R17 (cycle de vie) et R18 (dégradations liquidité/contre-swing #1c+#4) sont désormais CÂBLÉS et audités** dans `OB_Detector_v2.12.txt`. **R19 — audit de conformité v2.12 bouclé** : cœur du modèle FIDÈLE ; seule déviation réelle = le **DÉCLENCHEUR** (cluster A : pivot+taille vs cassure-corps externe + 1-2) → arbitrage humain à venir ; le **bug vault** (axiome YAML `degradation_zone_par_polarite`) est **CORRIGÉ** (réaligné sur transcripts). Ne restent, tous **NON-doctrinaux** : **R6** (géométrie fine du gap → validation visuelle TV) ; **inférences à valider** (formule morphologie base §T4 `body<mèche`) ; **câblage** résiduel du modèle de force (#2 liquidité-au-dessus déféré, #3 jeu interne non codable, cascade §2.6 — engineering) ; **déclencheur cluster A** (R19). Tout 🟡/⚠️ résiduel relève de l'un de ces quatre.
