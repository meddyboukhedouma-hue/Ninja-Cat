...
- **Config 4** (énergie inversée → OB d'origine seul) et **Config 5** (contre-structure → « OB du milieu ») : **= la séquence CHoCH-INVERSÉE** (« casse-bas PUIS recasse-haut »), **confirmées verbatim 2026-06-19** `NOAGAfn5VhQ 05:21:16–05:22:55` : *« tous les swings intermédiaires seront de la liquidité… le seul endroit où rentrer = l'order bloc d'origine… le jeu inversé = l'order block du milieu qui fait office d'order block principal »*. **RÉSOLUTION CLÉ** : l'« énergie CHoCH-inversée / presque annihilée » (`@ 05:20:32`) **n'est PAS un quantum de force** mais cette **règle de localisation** (les intermédiaires = liquidité/jeux internes → OB re-localisé : config 4 = OB d'origine seul ; config 5 = OB du milieu). Le seul « jeu interne » porteur d'un quantum est la **fausse cassure-mèche = ÷2** (`fausse_cassure_division`, `FFxKFvuYxrM 00:55:54`), **déjà neutralisée par la porte cassure-corps A2 (v2.13)** : les cassures-mèche ne créent plus d'OB. Reste à câbler = la **localisation configs 4/5** (détection contre-structure → OB du milieu), pas une dégradation de force.
...
| R14 | Localisation de l'OB d'origine dans la séquence (§2.5) | ✅ résolu 2026-06-19 : grille « **intérieur → 1ᵉʳ micro-swing / extérieur → 1ᵉʳ gros swing / même TF imbriqué → liquidité** » (`NOAGAfn5VhQ@02:56–03:02`, transcript + visuel « prenez la photo »). **Corrige le timestamp vault erroné `grille_ob_origine`@05:15.** Configs 1/2/3/6 déterministes ; **4/5 = la séquence CHoCH-inversée, CONFIRMÉES 2026-06-19** (`NOAGAfn5VhQ 05:21–05:22` : intermédiaires = liquidité, OB re-localisé — config 4 OB d'origine seul / config 5 OB du milieu) → **« énergie CHoCH-inversée » = cette règle de LOCALISATION, PAS un quantum de force** ; reste à CÂBLER (détection contre-structure). Le ÷2 « jeu interne » (fausse cassure-mèche) est déjà neutralisé par la porte cassure-corps A2 (v2.13) |
